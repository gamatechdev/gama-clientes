...
interface DocUnidade {
  ...
  unidade_id: number;
  unidade?: { nome_unidade: string, empresaid: string, clientes?: { nome_fantasia: string } }; // Join info
}
...
const ExamAssignmentModal = ({ isOpen, sectorName, sectorLinkId, onClose, loadingProp }: { isOpen: boolean, sectorName: string, sectorLinkId: number, onClose: () => void, loadingProp: boolean }) => {
  ...
  const handleSelectExam = (exame: Exame) => {
      setConfigExam({ id: exame.id, nome: exame.nome, isNew: false });
      setFormFlags({
          periodicidade: exame.periodicidade || 12,
          admissao: !!exame.admissao,
          demissao: !!exame.demissao,
          ret_trabalho: !!exame.ret_trabalho, 
          mud_riscos: !!exame.mud_riscos
      });
      setView('config');
  };

  const handleCreateNew = () => {
      setConfigExam({ nome: searchTerm, isNew: true });
      setFormFlags({
          periodicidade: 12,
          admissao: false,
          demissao: false,
  ...
  const handleSaveConfiguration = async () => {
      ...
      try {
          ...
          if (configExam.isNew) {
              const { data: newExame, error: createError } = await supabase.from('exames').insert({
                  nome: configExam.nome,
                  periodicidade: formFlags.periodicidade
              }).select().single();
          ...
          const { error: linkError } = await supabase.from('exames_unidade').insert({
  ...
  const handlePrepareCopy = async () => {
      setLoading(true);
      try {
          // 1. Get current link to find company
          const { data: currentLink } = await supabase
            .from('unidade_setor')
            .select('unidade:unidades(empresaid)')
            .eq('id', sectorLinkId)
            .single();
          
          // @ts-ignore
          const companyId = currentLink?.unidade?.empresaid;
          if (!companyId) throw new Error("Empresa não identificada.");

          // 2. Fetch all other sectors of this company
          const { data: sectors } = await supabase
            .from('unidade_setor')
            .select(`
                id,
                setor ( nome ),
                unidade!inner ( id, nome_unidade, empresaid )
            `)
            .eq('unidade.empresaid', companyId)
            .neq('id', sectorLinkId); // Exclude current

          if (sectors) {
              const formatted = sectors.map((s: any) => ({
                  id: s.id,
                  label: `${s.unidade.nome_unidade} > ${s.setor.nome}`
  ...
  const handleCopyExams = async () => {
      ...
      try {
          ...
          const { data: sourceExams } = await supabase
            .from('exames_unidade')
            .select('*')
            .eq('unidade_setor', selectedSourceId);

          if (!sourceExams || sourceExams.length === 0) {
              alert("O setor selecionado não possui exames cadastrados.");
              setSaving(false);
              return;
          }

          // 2. Filter out duplicates (already assigned to current sector)
          ...
          if (examsToCopy.length === 0) {
              alert("Todos os exames do setor de origem já estão cadastrados neste setor.");
              setSaving(false);
              return;
          }
          ...
          alert(`${examsToCopy.length} exames copiados com sucesso!`);
          setView('list');
          fetchAssignedExams();

      } catch (err: any) {
          console.error(err);
          alert("Erro ao copiar exames: " + err.message);
      } finally {
          setSaving(false);
      }
  };
    ...
    <div className="fixed inset-0 z-[120] flex items-center justify-center bg-black/40 backdrop-blur-sm animate-in fade-in duration-200 p-4">
      <GlassCard className="w-full max-w-2xl bg-white border-none shadow-2xl flex flex-col max-h-[85vh] p-0 overflow-hidden">
        ...
        <div className="flex-1 overflow-y-auto custom-scrollbar bg-[#f8fafc]">
           ...
           {/* VIEW 1: LIST ASSIGNED */}
           {view === 'list' && (
               <div className="p-6">
                   <div className="flex justify-between items-center mb-4 gap-2">
                       <h4 className="text-sm font-bold text-gray-500 uppercase">Exames Vinculados</h4>
                       <div className="flex gap-2">
                           <Button onClick={handlePrepareCopy} className="h-9 text-xs font-bold !bg-white !text-gray-600 border border-gray-200 !shadow-none hover:!bg-gray-50"><Copy size={14} className="mr-1"/> Copiar de Outro</Button>
                           <Button onClick={() => setView('search')} className="h-9 text-xs font-bold !bg-[#04a7bd] hover:!bg-[#038e9e] !shadow-none"><Plus size={14}/> Adicionar Exame</Button>
                       </div>
                   </div>
                   
                   {loading ? (
                       <div className="flex justify-center py-8"><div className="animate-spin rounded-full h-8 w-8 border-t-2 border-b-2 border-[#04a7bd]"></div></div>
                       ...
                       <div className="space-y-2">
                           {assignedExams.map(item => (
                               <div key={item.id} className="bg-white border border-gray-100 p-4 rounded-xl flex items-center justify-between shadow-sm">
                                   ...
                                   <button onClick={() => handleDeleteAssignment(item.id)} className="p-2 text-red-400 hover:bg-red-50 hover:text-red-600 rounded-lg transition-colors"><Trash2 size={16}/></button>
                               </div>
                           ))}
                       </div>
                   )}
                   ...
                   <div className="space-y-2">
                       {globalExams.map(ex => (
                           <div 
                               ...
                               className="p-3 bg-white border border-gray-100 hover:border-[#04a7bd] hover:shadow-sm rounded-xl cursor-pointer transition-all flex justify-between items-center group"
                           >
                               <span className="font-medium text-[#050a30] group-hover:text-[#04a7bd]">{ex.nome}</span>
                               <ArrowRight size={16} className="text-gray-300 group-hover:text-[#04a7bd]" />
                           </div>
                       ))}
                       
                       {searchTerm.length > 1 && globalExams.length === 0 && (
                           <div className="text-center py-8">
                               <p className="text-gray-400 mb-3">Exame não encontrado.</p>
                               <Button onClick={handleCreateNew} className="h-9 text-xs mx-auto"><Plus size={14}/> Criar "{searchTerm}"</Button>
                           </div>
                       )}
                   </div>
               </div>
                   ...
                   <div className="flex gap-4">
                        <div className="w-1/2">
                            ...
                            <input 
                                ...
                                onChange={(e) => setFormFlags({...formFlags, periodicidade: parseInt(e.target.value) || 0})}
                            />
                        </div>
                   </div>
                   ...
                   <div>
                        ...
                        <div className="grid grid-cols-2 gap-3">
                            <label className={`flex items-center gap-3 p-3 border rounded-xl cursor-pointer transition-all ${formFlags.admissao ? 'bg-blue-50 border-blue-200' : 'bg-white border-gray-200 hover:border-gray-300'}`}>
                                <input type="checkbox" checked={formFlags.admissao} onChange={(e) => setFormFlags({...formFlags, admissao: e.target.checked})} className="w-4 h-4 accent-[#04a7bd]" />
                            ...
                            <label className={`flex items-center gap-3 p-3 border rounded-xl cursor-pointer transition-all ${formFlags.demissao ? 'bg-blue-50 border-blue-200' : 'bg-white border-gray-200 hover:border-gray-300'}`}>
                                <input type="checkbox" checked={formFlags.demissao} onChange={(e) => setFormFlags({...formFlags, demissao: e.target.checked})} className="w-4 h-4 accent-[#04a7bd]" />
                            ...
                            <label className={`flex items-center gap-3 p-3 border rounded-xl cursor-pointer transition-all ${formFlags.ret_trabalho ? 'bg-blue-50 border-blue-200' : 'bg-white border-gray-200 hover:border-gray-300'}`}>
                                <input type="checkbox" checked={formFlags.ret_trabalho} onChange={(e) => setFormFlags({...formFlags, ret_trabalho: e.target.checked})} className="w-4 h-4 accent-[#04a7bd]" />
                            ...
                            <label className={`flex items-center gap-3 p-3 border rounded-xl cursor-pointer transition-all ${formFlags.mud_riscos ? 'bg-blue-50 border-blue-200' : 'bg-white border-gray-200 hover:border-gray-300'}`}>
                                <input type="checkbox" checked={formFlags.mud_riscos} onChange={(e) => setFormFlags({...formFlags, mud_riscos: e.target.checked})} className="w-4 h-4 accent-[#04a7bd]" />
        ...
        <div className="p-4 border-t border-gray-100 bg-white flex gap-3 shrink-0">
            {view === 'list' ? (
                ...
                <>
                    <Button onClick={() => setView('list')} className="flex-1 bg-gray-100 text-gray-600 hover:bg-gray-200 !shadow-none">Cancelar</Button>
                    
                    {view === 'config' && (
                        <Button onClick={handleSaveConfiguration} disabled={saving} className="flex-1 !bg-[#04a7bd] hover:!bg-[#038e9e] text-white">
                            {saving ? 'Salvando...' : 'Incluir Exame'}
                        </Button>
                    )}
...
const RiskAssignmentModal = ({ isOpen, sectorName, sectorLinkId, onClose, loadingProp }: { isOpen: boolean, sectorName: string, sectorLinkId: number, onClose: () => void, loadingProp: boolean }) => {
  ...
  const [selectedRiskIds, setSelectedRiskIds] = useState<number[]>([]); // Current selection state
  ...
  const fetchRisks = async () => {
    setLoading(true);
    try {
      // 1. Fetch available risks
      const { data: allRisks, error: rErr } = await supabase
        .from('riscos')
        .select('*')
        .order('nome');
      ...
      // 2. Fetch currently assigned risks
      ...
      const ids = assigned ? assigned.map(a => a.risco_id) : [];
      setInitialAssignedIds(ids);
      setSelectedRiskIds(ids);

    } catch (err) {
      console.error(err);
      alert('Erro ao carregar riscos.');
    } finally {
      setLoading(false);
    }
  };

  const toggleSelection = (riscoId: number) => {
  ...
  const handleCreateRisk = async () => {
    if (!newRiskData.nome) { alert("Nome é obrigatório"); return; }
    setSaving(true);
    try {
        const { data, error } = await supabase.from('riscos').insert({
            nome: newRiskData.nome,
            desc: newRiskData.desc,
            tipo: parseInt(newRiskData.tipo)
        }).select().single();

        if (error) throw error;

        // Add to local list and select it
        setRisks(prev => [...prev, data].sort((a,b) => a.nome.localeCompare(b.nome)));
        setSelectedRiskIds(prev => [...prev, data.id]);
        
        // Reset and go back
        setNewRiskData({ nome: '', desc: '', tipo: '6' });
        setMode('select');
        setSearchTerm(''); // Clear search to show context or keep it? Let's clear.

    } catch (err: any) {
        console.error(err);
        alert("Erro ao criar risco: " + err.message);
    } finally {
        setSaving(false);
    }
  };
  ...
  const handleConfirmChanges = async () => {
    ...
    try {
        // Calculate diffs
        const toAdd = selectedRiskIds.filter(id => !initialAssignedIds.includes(id));
        const toRemove = initialAssignedIds.filter(id => !selectedRiskIds.includes(id));

        if (toRemove.length > 0) {
            await supabase.from('riscos_unidade')
                .delete()
                .eq('unidade_setor', sectorLinkId)
                .in('risco_id', toRemove);
        }
        ...
        if (toAdd.length > 0) {
            ...
            await supabase.from('riscos_unidade').insert(payload);
        }

        alert("Riscos atualizados com sucesso!");
        onClose();
  ...
  // Logic to show risks: Only show if search term exists OR if it's already selected
  const visibleRisks = risks.filter(r => {
      const matchesSearch = searchTerm.length > 0 && r.nome.toLowerCase().includes(searchTerm.toLowerCase());
      const isAlreadySelected = selectedRiskIds.includes(r.id);
      
      // If searching, show matches. 
      // If not searching, show already selected items so user sees what's active.
      ...
      <GlassCard className="w-full max-w-2xl bg-white border-none shadow-2xl flex flex-col max-h-[85vh] p-0 overflow-hidden">
        ...
        <div className="p-6 border-b border-gray-100 flex justify-between items-center bg-gray-50/50 shrink-0">
           <div className="flex items-center gap-3">
              ...
              <div>
                 <h3 className="text-xl font-bold text-[#050a30]">
                    {mode === 'create' ? 'Cadastrar Novo Risco' : 'Gerenciar Riscos'}
                 </h3>
                 <p className="text-sm text-gray-500">{sectorName}</p>
              </div>
           </div>
        ...
        {/* Fixed Search Bar (Only visible in select mode and not loading) */}
        {!loading && mode === 'select' && (
            <div className="p-4 bg-white border-b border-gray-100 z-10 shrink-0">
                <Input 
        ...
        <div className="flex-1 overflow-y-auto custom-scrollbar bg-[#f8fafc]">
           {loading ? (
             ...
             <div className="p-6 space-y-5">
                <Input 
                    label="Nome do Risco" 
                    placeholder="Ex: Ruído Intermitente" 
                    value={newRiskData.nome} 
                    onChange={(e) => setNewRiskData({...newRiskData, nome: e.target.value})} 
                />
                <div className="w-full">
                    <label className="block text-sm text-[#050a30]/80 mb-2 ml-1 font-medium">Descrição Detalhada</label>
                    <textarea 
                        className="w-full bg-white/60 border border-gray-200/60 text-[#050a30] rounded-2xl py-3 px-4 focus:outline-none focus:border-[#04a7bd] focus:bg-white transition-all min-h-[100px] resize-none"
                        placeholder="Descreva o risco..."
                        value={newRiskData.desc}
                        onChange={(e) => setNewRiskData({...newRiskData, desc: e.target.value})}
                    />
                </div>
                <div>
                    ...
                    <div className="relative">
                        <select 
                            className="w-full bg-white border border-gray-200 text-[#050a30] rounded-xl py-3 px-4 pr-10 focus:outline-none focus:border-[#04a7bd] appearance-none"
                            value={newRiskData.tipo}
                            onChange={(e) => setNewRiskData({...newRiskData, tipo: e.target.value})}
                        >
                            {RISK_TYPES_MAP.map(t => (
                ...
                {visibleRisks.length === 0 ? (
                    ...
                    <div className="space-y-2">
                        {visibleRisks.map(risk => {
                            ...
                            const riskType = getRiskLabel(risk.tipo);
                            ...
                            return (
                                <div 
                                    ...
                                    className={`
                                        p-3 rounded-xl border flex items-center justify-between cursor-pointer transition-all hover:shadow-sm
                                        ${isSelected ? 'bg-red-50 border-red-200' : 'bg-white border-gray-100 hover:border-gray-200'}
                                    `}
                                >
                                    <div className="flex items-center gap-3 overflow-hidden">
                                        <div className={`w-8 h-8 rounded-lg flex items-center justify-center text-xs font-bold shrink-0 ${riskType?.color.replace('text-', 'text-opacity-80 text-')}`}>
                                            {riskType?.label.charAt(0)}
                                        </div>
                                        <div className="min-w-0">
                                            <p className={`text-sm font-bold truncate ${isSelected ? 'text-red-800' : 'text-[#050a30]'}`}>{risk.nome}</p>
                                            <p className="text-[10px] text-gray-400 truncate">{riskType?.label}</p>
                                        </div>
                                    </div>
        ...
        <div className="p-4 border-t border-gray-100 bg-white flex gap-3 shrink-0">
            {mode === 'create' ? (
                <>
                    <Button onClick={() => setMode('select')} className="flex-1 bg-gray-100 text-gray-600 hover:bg-gray-200 !shadow-none">Cancelar</Button>
                    <Button onClick={handleCreateRisk} disabled={saving} className="flex-1">{saving ? 'Salvando...' : 'Salvar Risco'}</Button>
                </>
            ) : (
                <>
                    <Button onClick={onClose} className="flex-1 bg-gray-100 text-gray-600 hover:bg-gray-200 !shadow-none">Cancelar</Button>
                    <Button onClick={handleConfirmChanges} disabled={saving} className="flex-1 !bg-red-600 hover:!bg-red-700 text-white shadow-red-500/20">{saving ? 'Salvando...' : 'Confirmar Alterações'}</Button>
                </>
            )}
        </div>
    ...
    <div className="fixed inset-0 z-[110] flex items-center justify-center bg-black/40 backdrop-blur-sm animate-in fade-in duration-200 p-4">
      <GlassCard className="w-full max-w-lg bg-white border-none shadow-2xl flex flex-col max-h-[85vh] p-0 overflow-hidden">
        <div className="p-6 border-b border-gray-100 flex justify-between items-center bg-gray-50/50">
           <div className="flex items-center gap-3">
              <div className="p-2 bg-[#04a7bd]/10 rounded-lg text-[#04a7bd]"><History size={24} /></div>
              <div>
                 <h3 className="text-xl font-bold text-[#050a30]">{colabName}</h3>
                 <p className="text-sm text-gray-500">Histórico de ASOs</p>
              </div>
           </div>
           <button onClick={onClose} className="p-2 hover:bg-gray-200 rounded-full transition-colors text-gray-500">
              <X size={24} />
             ...
             <div className="space-y-3">
                {data.length === 0 ? (
                  <div className="text-center py-8 text-gray-400">
                    ...
                    <p>Nenhum ASO liberado encontrado.</p>
                  </div>
                ) : (
                  data.map((aso) => (
                    <div key={aso.id} className="bg-white p-4 rounded-xl border border-gray-100 shadow-sm flex items-center justify-between hover:border-[#04a7bd]/30 transition-all">
                       <div className="flex items-center gap-4">
                          <div className="flex flex-col items-center justify-center bg-gray-50 rounded-lg w-12 h-12 text-[#050a30] border border-gray-200">
                             <span className="text-[10px] font-bold uppercase">{new Date(aso.data_atendimento).toLocaleDateString('pt-BR', {month: 'short'}).replace('.','')}</span>
                             <span className="text-lg font-bold">{new Date(aso.data_atendimento).getDate()}</span>
                          </div>
                          <div>
                             <p className="text-sm font-bold text-[#050a30]">{aso.tipo}</p>
                             <p className="text-xs text-gray-400">Realizado em {new Date(aso.data_atendimento).getFullYear()}</p>
                          </div>
                       </div>
                       <Button 
                         onClick={() => window.open(aso.aso_url, '_blank')} 
                         className="h-9 px-3 text-xs font-bold !bg-[#04a7bd] hover:!bg-[#038e9e] !shadow-none"
                       >
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/40 backdrop-blur-sm animate-in fade-in duration-200 p-4">
      <GlassCard className="w-full max-w-3xl bg-white border-none shadow-2xl flex flex-col max-h-[85vh] p-0 overflow-hidden">
        <div className="p-6 border-b border-gray-100 flex justify-between items-center bg-gray-50/50">
           <div className="flex items-center gap-3">
              <div className={`p-2 rounded-lg ${type === 'aso_pending' ? 'bg-orange-100 text-orange-600' : type === 'aso_expiring' ? 'bg-purple-100 text-purple-600' : 'bg-red-100 text-red-600'}`}>
                 {type === 'aso_pending' ? <Clock size={24} /> : type === 'aso_expiring' ? <Users size={24} /> : <AlertCircle size={24} />}
              </div>
              <div>
        ...
        <div className="p-0 overflow-y-auto custom-scrollbar bg-[#f8fafc]">
           <table className="w-full text-left border-collapse">
              <thead className="bg-gray-100/50 sticky top-0 z-10 text-xs font-bold text-gray-500 uppercase tracking-wider">
                 <tr>
                    <th className="p-4 border-b border-gray-100">Nome / Documento</th>
                    <th className="p-4 border-b border-gray-100">Unidade</th>
                    <th className="p-4 border-b border-gray-100 text-right">Data / Status</th>
                 </tr>
              </thead>
              <tbody className="text-sm text-[#050a30]">
                 {data.map((item, idx) => (
                    <tr key={idx} className="border-b border-gray-50 hover:bg-white transition-colors">
                       <td className="p-4 font-medium">
                          {type === 'aso_pending' && item.colaborador?.nome}
                          {type === 'aso_expiring' && item.colaborador_nome}
                          {type === 'docs' && (
                             <div className="flex flex-col">
                                <span>{UNIT_DOC_TYPES.find(t => t.id === item.tipo_doc)?.name || 'Doc'}</span>
                                <span className="text-[10px] text-gray-400 font-normal">Validade: {item.validade ? new Date(item.validade).toLocaleDateString('pt-BR') : 'N/A'}</span>
                             </div>
                          )}
                       </td>
                       <td className="p-4 text-gray-500">
                          {type === 'aso_pending' && item.unidade_info?.nome_unidade}
                          {type === 'aso_expiring' && item.unidade_nome}
                          {type === 'docs' && item.unidade?.nome_unidade}
                       </td>
                       <td className="p-4 text-right">
                          ...
                          {type === 'aso_expiring' && (
                             <div className="flex flex-col items-end">
                                <span className={`inline-flex items-center gap-1 px-2 py-1 rounded-full text-xs font-bold ${item.status === 'expired' ? 'bg-red-50 text-red-600' : 'bg-yellow-50 text-yellow-600'}`}>
                                   {item.status === 'expired' ? 'Vencido' : 'Vence em breve'}
                                </span>
                          ...
                          {type === 'docs' && (
                             <span className="inline-flex items-center gap-1 px-2 py-1 rounded-full bg-red-50 text-red-600 text-xs font-bold">
                                <AlertTriangle size={12} /> Verificar
                             </span>
                          )}
                       </td>
                    </tr>
                 ...
                 {data.length === 0 && (
                    <tr>
                       <td colSpan={3} className="p-8 text-center text-gray-400 italic">Nenhum registro encontrado.</td>
                    </tr>
                 )}
              </tbody>
           </table>
    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/40 backdrop-blur-sm animate-in fade-in duration-200">
      <GlassCard className="w-full max-w-md p-6 bg-white border-none shadow-2xl">
        <div className="flex items-center gap-3 mb-6">
           <div className="p-3 bg-[#04a7bd]/10 rounded-full text-[#04a7bd]"><UserPlus size={24} /></div>
           <div>
              <h3 className="text-xl font-bold text-[#050a30]">Criar Acesso</h3>
              <p className="text-xs text-gray-500 max-w-[250px] truncate">Para: {company.nome_fantasia}</p>
           </div>
        </div>
        ...
        <div className="space-y-4 mb-6">
           ...
           <div>
              ...
              <label htmlFor="logo-upload" className={`w-full flex items-center justify-center gap-3 py-4 rounded-xl border-2 border-dashed transition-all cursor-pointer ${selectedFile ? 'border-[#04a7bd] bg-[#04a7bd]/5 text-[#04a7bd]' : 'border-gray-200 bg-gray-50 text-gray-400 hover:border-[#04a7bd]/50'}`}>
                 <Camera size={20} />
                 <span className="text-sm font-medium">{selectedFile ? selectedFile.name : 'Selecionar Imagem'}</span>
              </label>
           </div>
        </div>
        ...
        <div className="flex gap-3 w-full">
          ...
          <button onClick={handleCreate} disabled={isSubmitting || loading} className="flex-1 py-3 rounded-xl bg-[#04a7bd] font-medium text-white hover:bg-[#038e9e] transition-colors shadow-lg shadow-[#04a7bd]/20 disabled:opacity-70 flex items-center justify-center gap-2">
            {isSubmitting ? 'Processando...' : 'Criar Usuário'}
          </button>
        </div>
      </GlassCard>
    </div>
...
const UnitDocsModal = ({ isOpen, unitId, unitName, docs, onClose, onUpdate, loading }: { isOpen: boolean, unitId: number, unitName: string, docs: DocUnidade[], onClose: () => void, onUpdate: () => void, loading: boolean }) => {
  ...
  const handleStartEdit = (doc: DocUnidade | undefined, typeId: number) => {
    setEditingType(typeId);
    setEditUrl(doc?.doc_url || '');
    setEditDate(doc?.validade || '');
    setSelectedFile(null);
  };
  ...
  const getFolderName = (typeId: number) => {
    switch (typeId) {
        case 57: return 'PGR';
        case 56: return 'PCMSO';
        case 53: return 'LTCAT';
        default: return 'OTHERS';
    }
  };

  const handleSave = async (typeId: number, existingId?: number) => {
    if ((!editUrl && !selectedFile) || !editDate) {
        alert("Por favor, selecione um arquivo e a Data de Validade.");
        return;
    }
    ...
    try {
        ...
        if (selectedFile) {
            const folder = getFolderName(typeId);
            const cleanFileName = selectedFile.name.replace(/[^a-zA-Z0-9.]/g, '_');
            const fileName = `${unitId}_${Date.now()}_${cleanFileName}`;
        ...
        if (existingId) {
            const { error } = await supabase.from('doc_unidade').update(payload).eq('id', existingId);
            if (error) throw error;
        } else {
            const { error } = await supabase.from('doc_unidade').insert(payload);
            if (error) throw error;
        }
  ...
  const getDocStatus = (doc: DocUnidade | undefined) => {
    if (!doc || !doc.doc_url) return { status: 'missing', label: 'Indisponível', color: 'bg-gray-100 text-gray-500', icon: AlertTriangle };
    ...
    if (doc.validade) {
      ...
      if (valDateLocal < today) return { status: 'expired', label: 'Vencido', color: 'bg-red-100 text-red-600', icon: AlertTriangle };
      
      const diffTime = valDateLocal.getTime() - today.getTime();
      const diffDays = Math.ceil(diffTime / (1000 * 60 * 60 * 24));
      
      if (diffDays <= 30) return { status: 'warning', label: 'Vencendo', color: 'bg-yellow-100 text-yellow-700', icon: Clock };
    }

    return { status: 'valid', label: 'Vigente', color: 'bg-green-100 text-green-700', icon: CheckCircle };
  };
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/40 backdrop-blur-sm animate-in fade-in duration-200 p-4">
      <GlassCard className="w-full max-w-2xl bg-white border-none shadow-2xl flex flex-col max-h-[85vh] p-0 overflow-hidden">
        <div className="p-6 border-b border-gray-100 flex justify-between items-center bg-gray-50/50">
           <div className="flex items-center gap-3">
              <div className="p-2 bg-[#04a7bd]/10 rounded-lg text-[#04a7bd]"><FolderOpen size={24} /></div>
              <div>
                 <h3 className="text-xl font-bold text-[#050a30]">Documentos da Unidade</h3>
        ...
        <div className="p-6 overflow-y-auto custom-scrollbar bg-[#f8fafc]">
           {loading ? (
             ...
             <div className="grid grid-cols-1 gap-4">
                {UNIT_DOC_TYPES.map(type => {
                   const doc = docs.find(d => d.tipo_doc === type.id);
                   ...
                   if (isEditing) {
                       return (
                           <div key={type.id} className="bg-white p-5 rounded-2xl border-2 border-[#04a7bd]/30 shadow-md space-y-4">
                               <div className="flex items-center gap-2 mb-2">
                                   <div className="p-2 bg-[#04a7bd]/10 rounded-lg text-[#04a7bd]"><Edit2 size={18} /></div>
                                   <h4 className="font-bold text-[#050a30]">Editando {type.name}</h4>
                               </div>
                               <div className="space-y-4">
                                   <div>
                                       <label className="text-xs font-bold text-gray-500 uppercase ml-1 mb-1 block">Arquivo do Documento</label>
                                       <input type="file" id={`file-upload-${type.id}`} className="hidden" onChange={handleFileChange} accept=".pdf,.doc,.docx,.jpg,.jpeg,.png" />
                                       <div className="flex gap-2">
                                           <label htmlFor={`file-upload-${type.id}`} className="flex-1 cursor-pointer flex items-center justify-center gap-2 py-3 px-4 rounded-xl bg-gray-50 border border-gray-200 border-dashed hover:border-[#04a7bd] hover:bg-[#04a7bd]/5 transition-all text-gray-500 hover:text-[#04a7bd] text-sm font-medium">
                                               <UploadCloud size={18} />
                                               <span className="truncate">{selectedFile ? selectedFile.name : (editUrl ? 'Substituir Arquivo' : 'Selecionar Arquivo')}</span>
                                           </label>
                                           {(editUrl || selectedFile) && (<div className="flex items-center justify-center w-12 bg-green-50 text-green-600 rounded-xl border border-green-100" title="Arquivo Pronto"><CheckCircle size={20} /></div>)}
                                       </div>
                                       {!selectedFile && editUrl && (<div className="mt-1 px-1 flex items-center gap-1 text-[10px] text-gray-400"><LinkIcon size={10} /><span className="truncate max-w-xs" title={editUrl}>Link Atual: ...{editUrl.slice(-20)}</span></div>)}
                                   </div>
                                   <div>
                                       <label className="text-xs font-bold text-gray-500 uppercase ml-1 mb-1 block">Data de Validade</label>
                                       <div className="relative">
                                           <div className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400"><Calendar size={16} /></div>
                                           <input type="date" value={editDate} onChange={(e) => setEditDate(e.target.value)} className="w-full pl-9 pr-3 py-2.5 rounded-xl bg-gray-50 border border-gray-200 focus:border-[#04a7bd] focus:outline-none text-sm transition-colors" />
                                       </div>
                                   </div>
                               </div>
                               <div className="flex gap-2 pt-2">
                                   <Button onClick={handleCancelEdit} className="flex-1 h-9 text-xs !bg-gray-100 !text-gray-600 !shadow-none hover:!bg-gray-200">Cancelar</Button>
                                   <Button onClick={() => handleSave(type.id, doc?.id)} disabled={saving} className="flex-1 h-9 text-xs">{saving ? 'Enviando...' : 'Salvar Documento'}</Button>
                               </div>
                           </div>
                       );
                   }
                   ...
                   return (
                     <div key={type.id} className="bg-white p-5 rounded-2xl border border-gray-100 shadow-sm flex flex-col md:flex-row md:items-center justify-between gap-4 hover:shadow-md transition-shadow group">
                        <div className="flex items-start gap-4">
                           <div className={`p-3 rounded-xl flex items-center justify-center ${status.color}`}><FileText size={24} /></div>
                           <div>
                              <div className="flex items-center gap-2 mb-1">
                                 <h4 className="text-lg font-bold text-[#050a30]">{type.name}</h4>
                                 <span className={`text-[10px] font-bold uppercase px-2 py-0.5 rounded-md flex items-center gap-1 ${status.color}`}><StatusIcon size={10} /> {status.label}</span>
                              </div>
                              <p className="text-xs text-gray-500 mb-1">{type.fullName}</p>
                              {doc?.validade && (<p className="text-xs font-mono text-gray-400">Validade: {formatDate(doc.validade)}</p>)}
                           </div>
                        </div>
                        <div className="flex items-center gap-2">
                           {doc?.doc_url ? (<Button onClick={() => window.open(doc.doc_url || '', '_blank')} className="h-10 text-xs font-bold px-4 !bg-[#04a7bd] hover:!bg-[#038e9e] !shadow-none whitespace-nowrap"><Download size={14} /> Baixar</Button>) : (<button disabled className="h-10 px-4 rounded-xl bg-gray-100 text-gray-400 text-xs font-bold cursor-not-allowed flex items-center gap-2"><X size={14} /> Pendente</button>)}
                           <button onClick={() => handleStartEdit(doc, type.id)} className="w-10 h-10 flex items-center justify-center rounded-xl bg-gray-50 text-gray-400 hover:bg-[#04a7bd] hover:text-white transition-all shadow-sm border border-gray-100" title={doc ? "Substituir Documento" : "Incluir Documento"}>{doc ? <Edit2 size={16} /> : <UploadCloud size={18} />}</button>
                        </div>
                     </div>
                   );
                })}
    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/40 backdrop-blur-sm animate-in fade-in duration-200">
      <GlassCard className="w-full max-w-sm p-6 bg-white border-none shadow-2xl scale-100">
        <div className="flex flex-col items-center text-center">
          <div className="w-12 h-12 bg-red-100 rounded-full flex items-center justify-center mb-4 text-red-600"><AlertTriangle size={24} /></div>
          <h3 className="text-xl font-bold text-[#050a30] mb-2">{title}</h3>
          <p className="text-sm text-[#050a30]/60 mb-6">{message}</p>
          <div className="flex gap-3 w-full">
            <button onClick={onCancel} className="flex-1 py-2.5 rounded-xl bg-gray-100 font-medium text-gray-600 hover:bg-gray-200 transition-colors">Cancelar</button>
            <button onClick={onConfirm} disabled={loading} className="flex-1 py-2.5 rounded-xl bg-red-500 font-medium text-white hover:bg-red-600 transition-colors shadow-lg shadow-red-500/20 disabled:opacity-70">{loading ? 'Apagando...' : 'Apagar'}</button>
          </div>
        </div>
      </GlassCard>
    </div>
...
const PeriodicityModal = ({ isOpen, targetName, currentPeriodicity, onConfirm, onCancel, loading }: any) => {
  ...
  useEffect(() => {
    if (isOpen) {
      if ([6, 12, 24].includes(currentPeriodicity)) {
        setSelectedOption(currentPeriodicity.toString());
        setManualValue('');
      } else if (currentPeriodicity) {
        setSelectedOption('manual');
        setManualValue(currentPeriodicity.toString());
      } else {
        setSelectedOption('');
        setManualValue('');
      }
    }
  }, [isOpen, currentPeriodicity]);
  ...
  const handleSave = () => {
    ...
    if (selectedOption === 'manual') {
      finalValue = parseInt(manualValue, 10);
    } else {
      finalValue = parseInt(selectedOption, 10);
    }

    if (!isNaN(finalValue) && finalValue > 0) {
      onConfirm(finalValue);
    } else {
      alert("Por favor insira um valor válido de meses.");
    }
  };
    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/40 backdrop-blur-sm animate-in fade-in duration-200">
      <GlassCard className="w-full max-w-sm p-6 bg-white border-none shadow-2xl">
        <div className="flex items-center gap-3 mb-4">
           <div className="p-2 bg-[#04a7bd]/10 rounded-lg text-[#04a7bd]"><Activity size={24} /></div>
           <div>
              <h3 className="text-lg font-bold text-[#050a30]">Periodicidade de Exames</h3>
              <p className="text-xs text-gray-500 truncate max-w-[200px]">{targetName}</p>
           </div>
        </div>
        <div className="space-y-4 mb-6">
           <div className="relative">
              <label className="text-xs font-bold text-gray-500 uppercase tracking-wide ml-1 mb-1 block">Frequência</label>
              <div className="relative">
                <select className="w-full bg-gray-50 border border-gray-200 text-[#050a30] rounded-xl py-3 px-4 pr-10 focus:outline-none focus:border-[#04a7bd] appearance-none" value={selectedOption} onChange={(e) => setSelectedOption(e.target.value)}>
                   <option value="" disabled>Selecione...</option>
                   <option value="6">Semestral (6 meses)</option>
                   <option value="12">Anual (12 meses)</option>
                   <option value="24">Bienal (24 meses)</option>
                   <option value="manual">Inserir meses...</option>
                </select>
                <div className="absolute right-4 top-1/2 -translate-y-1/2 pointer-events-none text-gray-400"><ChevronDown size={16} /></div>
              </div>
           </div>
           {selectedOption === 'manual' && (<Input type="number" label="Quantidade de Meses" placeholder="Ex: 3" value={manualValue} onChange={(e) => setManualValue(e.target.value)} autoFocus />)}
        </div>
        <div className="flex gap-3 w-full">
          <button onClick={onCancel} className="flex-1 py-2.5 rounded-xl bg-gray-100 font-medium text-gray-600 hover:bg-gray-200 transition-colors">Cancelar</button>
          <button onClick={handleSave} disabled={loading} className="flex-1 py-2.5 rounded-xl bg-[#04a7bd] font-medium text-white hover:bg-[#038e9e] transition-colors shadow-lg shadow-[#04a7bd]/20 disabled:opacity-70">{loading ? 'Salvando...' : 'Salvar'}</button>
        </div>
      </GlassCard>
    </div>
  );
...
const EditCargoModal = ({ isOpen, cargoName, cargoDescription, onConfirm, onCancel, loading }: any) => {
  ...
  useEffect(() => {
    if (isOpen) {
      setName(cargoName || '');
      setDescription(cargoDescription || '');
    }
  }, [isOpen, cargoName, cargoDescription]);
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/40 backdrop-blur-sm animate-in fade-in duration-200">
      <GlassCard className="w-full max-w-md p-6 bg-white border-none shadow-2xl">
        <div className="flex items-center gap-3 mb-4">
           <div className="p-2 bg-amber-500/10 rounded-lg text-amber-500"><Edit2 size={24} /></div>
           <div>
              <h3 className="text-lg font-bold text-[#050a30]">Editar Cargo</h3>
              <p className="text-xs text-gray-500">Detalhes do cargo neste setor</p>
           </div>
        </div>
        <div className="space-y-4 mb-6">
          <Input label="Nome do Cargo" placeholder="Nome do cargo" value={name} onChange={(e) => setName(e.target.value)} />
          <div className="w-full">
            <label className="block text-sm text-[#050a30]/80 mb-2 ml-1 font-medium">Descrição de Atividade</label>
            <textarea className="w-full bg-white/60 border border-gray-200/60 text-[#050a30] placeholder-gray-400 rounded-2xl py-3.5 px-4 focus:outline-none focus:border-[#04a7bd] focus:bg-white focus:ring-2 focus:ring-[#04a7bd]/20 transition-all duration-200 backdrop-blur-sm shadow-sm min-h-[100px] resize-none" placeholder="Descreva as atividades exercidas..." value={description} onChange={(e) => setDescription(e.target.value)} />
          </div>
        </div>
        <div className="flex gap-3 w-full">
          <button onClick={onCancel} className="flex-1 py-2.5 rounded-xl bg-gray-100 font-medium text-gray-600 hover:bg-gray-200 transition-colors">Cancelar</button>
          <button onClick={() => onConfirm(name, description)} disabled={loading || !name.trim()} className="flex-1 py-2.5 rounded-xl bg-[#04a7bd] font-medium text-white hover:bg-[#038e9e] transition-colors shadow-lg shadow-[#04a7bd]/20 disabled:opacity-70">{loading ? 'Salvando...' : 'Salvar Alterações'}</button>
        </div>
      </GlassCard>
    </div>
  );
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/40 backdrop-blur-sm animate-in fade-in duration-200">
      <GlassCard className="w-full max-w-sm p-6 bg-white border-none shadow-2xl">
        ...
        <div className="flex gap-3 w-full">
          <button onClick={onCancel} className="flex-1 py-2.5 rounded-xl bg-gray-100 font-medium text-gray-600 hover:bg-gray-200 transition-colors">Cancelar</button>
          <button onClick={() => onConfirm(value)} disabled={loading || !value.trim()} className="flex-1 py-2.5 rounded-xl bg-[#04a7bd] font-medium text-white hover:bg-[#038e9e] transition-colors shadow-lg shadow-[#04a7bd]/20 disabled:opacity-70">{loading ? 'Salvando...' : 'Salvar'}</button>
        </div>
      </GlassCard>
    </div>
  );
...
const AddEntityModal = ({ isOpen, title, entityLabel, availableOptions, onConfirm, onCancel, loading }: any) => {
  ...
  const handleConfirm = () => {
    if (mode === 'create' && newValue.trim()) {
      onConfirm({ mode: 'create', value: newValue });
    } else if (mode === 'select' && selectedId) {
      onConfirm({ mode: 'select', id: selectedId });
    }
  };
    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/40 backdrop-blur-sm animate-in fade-in duration-200">
      <GlassCard className="w-full max-w-md p-6 bg-white border-none shadow-2xl">
        ...
        <div className="flex bg-gray-100 p-1 rounded-xl mb-6">
          <button onClick={() => setMode('select')} className={`flex-1 py-2 rounded-lg text-sm font-medium transition-all ${mode === 'select' ? 'bg-white text-[#04a7bd] shadow-sm' : 'text-gray-500'}`}>Selecionar Existente</button>
          <button onClick={() => setMode('create')} className={`flex-1 py-2 rounded-lg text-sm font-medium transition-all ${mode === 'create' ? 'bg-white text-[#04a7bd] shadow-sm' : 'text-gray-500'}`}>Cadastrar Novo</button>
        </div>
        <div className="min-h-[100px]">
            {mode === 'select' ? (
                <div className="space-y-2">
                    <label className="text-sm font-medium text-gray-600 ml-1">Buscar {entityLabel}</label>
                    {availableOptions && availableOptions.length > 0 ? (
                        <div className="relative">
                            <select className="w-full bg-gray-50 border border-gray-200 text-[#050a30] rounded-xl py-3 px-4 pr-10 focus:outline-none focus:border-[#04a7bd] appearance-none transition-colors" value={selectedId} onChange={(e) => setSelectedId(e.target.value)}>
                                <option value="">Selecione...</option>
                                {availableOptions.map((opt: any) => (<option key={opt.id} value={opt.id}>{opt.nome || opt.nome_unidade}</option>))}
                            </select>
                            <div className="absolute right-4 top-1/2 -translate-y-1/2 pointer-events-none text-gray-400"><ChevronDown size={16} /></div>
                        </div>
                    ) : (<div className="p-4 bg-yellow-50 text-yellow-700 text-sm rounded-xl border border-yellow-100 flex items-center gap-2"><AlertTriangle size={16} /> Não há {entityLabel}s disponíveis.</div>)}
                </div>
            ) : (<Input label={`Nome do ${entityLabel}`} placeholder={`Ex: Novo ${entityLabel}`} value={newValue} onChange={(e) => setNewValue(e.target.value)} autoFocus />)}
        </div>
        <div className="flex gap-3 w-full mt-6">
          <button onClick={onCancel} className="flex-1 py-3 rounded-xl bg-gray-100 font-medium text-gray-600 hover:bg-gray-200 transition-colors">Cancelar</button>
          <button onClick={handleConfirm} disabled={loading || (mode === 'create' && !newValue.trim()) || (mode === 'select' && !selectedId)} className="flex-1 py-3 rounded-xl bg-[#04a7bd] font-medium text-white hover:bg-[#038e9e] transition-colors shadow-lg shadow-[#04a7bd]/20 disabled:opacity-70 disabled:cursor-not-allowed">{loading ? 'Salvando...' : 'Salvar'}</button>
        </div>
      </GlassCard>
    </div>
  );
...
export default function Dashboard({ session }: DashboardProps) {
  ...
  const [activeTab, setActiveTab] = useState<'dashboard' | 'companies'>('companies');
  const [view, setView] = useState<'companies' | 'units' | 'sectors' | 'cargos'>('companies');
  ...
  const refreshCurrentView = async () => {
    ...
    if (showHierarchy && hierarchyCompany) {
  ...
  const openHierarchy = async (company: Cliente) => {
    if (!showHierarchy) setShowHierarchy(true);
  ...
  // ... (Keeping CRUD functions same as before) ...
  ...
  // ... (Other CRUD and trigger functions from previous code kept intact) ...
  ...
  const handleBack = () => { if (view === 'cargos') { setSelectedSector(null); setSectorCargos([]); setView('sectors'); } else if (view === 'sectors') { setSelectedUnit(null); setUnitSectors([]); setView('units'); } else if (view === 'units') { setSelectedCompany(null); setUnits([]); setView('companies'); } };
  const filteredCompanies = companies.filter(c => c.nome_fantasia?.toLowerCase().includes(searchTerm.toLowerCase()));
  
  const Sidebar = () => (
      <aside className={`fixed inset-y-0 left-0 z-40 w-80 bg-white/80 backdrop-blur-xl border-r border-gray-200 transform transition-transform duration-300 ease-in-out lg:translate-x-0 lg:static flex flex-col ${isSidebarOpen ? 'translate-x-0 shadow-2xl' : '-translate-x-full'}`}>
         <div className="p-8 border-b border-gray-200/60 flex flex-col items-center text-center">
            <div className="w-20 h-20 rounded-2xl bg-[#04a7bd]/10 flex items-center justify-center text-[#04a7bd] mb-4"><Building2 size={36} /></div>
         ...
         <nav className="flex-1 p-5 space-y-2">
            <button onClick={() => { setActiveTab('companies'); setIsSidebarOpen(false); }} className={`w-full flex items-center gap-3 px-5 py-4 rounded-xl transition-all duration-300 group ${activeTab === 'companies' ? 'bg-[#04a7bd] text-white shadow-lg shadow-[#04a7bd]/20 font-bold' : 'text-gray-600 hover:bg-gray-50 hover:text-[#050a30] font-medium'}`}><Building2 size={22} /><span>Empresas</span>{activeTab === 'companies' && <ChevronRight size={18} className="ml-auto opacity-80" />}</button>
            <button onClick={() => { setActiveTab('dashboard'); setIsSidebarOpen(false); }} className={`w-full flex items-center gap-3 px-5 py-4 rounded-xl transition-all duration-300 group ${activeTab === 'dashboard' ? 'bg-[#04a7bd] text-white shadow-lg shadow-[#04a7bd]/20 font-bold' : 'text-gray-600 hover:bg-gray-50 hover:text-[#050a30] font-medium'}`}><PieChart size={22} /><span>Visão Geral</span>{activeTab === 'dashboard' && <ChevronRight size={18} className="ml-auto opacity-80" />}</button>
         </nav>
         <div className="p-5 border-t border-gray-200/60 bg-gray-50/50">
           <button onClick={() => supabase.auth.signOut()} className="w-full flex items-center justify-center gap-2 text-red-600 font-bold hover:bg-red-50 py-4 rounded-xl transition-colors border border-transparent hover:border-red-100"><LogOut size={20} /><span>Sair</span></button>
         </div>
      </aside>
  );
  ...
  return (
    <div className="flex h-screen w-screen bg-[#f8fafc] overflow-hidden">
      ...
      <main className="flex-1 flex flex-col h-full overflow-hidden relative bg-[#f8fafc] mesh-bg">
        ...
        <div className="flex-1 overflow-y-auto custom-scrollbar p-6 lg:p-10 w-full">
           {activeTab === 'dashboard' && (
      ...
      {/* HIERARCHY MODAL - REVERTED TO STANDARD SCROLLABLE CANVAS */}
      {showHierarchy && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-md animate-in fade-in duration-300">
          <GlassCard className="w-full h-full max-w-[95vw] max-h-[90vh] flex flex-col p-0 overflow-hidden shadow-2xl bg-white/95 relative">
            
            <div className="absolute top-0 left-0 right-0 p-6 border-b border-gray-100 flex justify-between items-center bg-white/80 backdrop-blur-lg z-20">
              <div className="flex items-center gap-3">
                 <div className="p-2 bg-[#04a7bd]/10 rounded-lg"><Network size={24} className="text-[#04a7bd]" /></div>
                 <div><h2 className="text-xl font-bold text-[#050a30]">Mapa Organizacional</h2><p className="text-sm text-[#050a30]/60">{hierarchyCompany?.nome_fantasia}</p></div>
              </div>
              <button onClick={() => setShowHierarchy(false)} className="p-2 hover:bg-gray-200/50 rounded-full transition-colors"><X size={24} className="text-[#050a30]/60" /></button>
            </div>
            
            <div className="flex-1 overflow-auto custom-scrollbar bg-[#f8fafc] relative pt-20">
               {/* Grid Background */}
               <div className="absolute inset-0 pointer-events-none opacity-[0.05]" style={{ backgroundImage: 'radial-gradient(#050a30 1px, transparent 1px)', backgroundSize: '30px 30px' }}></div>
               ...
               {hierarchyLoading ? (
                 ...
                 <div className="flex flex-col items-center justify-center text-[#050a30]/40 h-full">
                 ...
                 <div className="flex items-center justify-start p-20 min-w-max min-h-full">
                    {/* Root: Company */}
