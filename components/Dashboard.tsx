...
// --- Icons & Helpers ---
const ActionButton = memo(({ onClick, icon: Icon, colorClass, title }: any) => (
  <button
...
// --- Memoized Hierarchy Sub-Components ---

interface CargoNodeProps {
  cargo: any;
  unitId: number;
  expandedColabs: Record<string, any[]>;
  loadingColabs: Record<string, boolean>;
  onToggleCollaborators: (unitId: number, cargoId: number) => void;
  onViewHistory: (colabId: string, colabName: string) => void;
  onTriggerPeriodicityCargo: (cargo: any) => void;
  onTriggerEditCargoDetails: (cargo: any) => void;
        ...
        <div className="ml-16 mt-2 animate-in slide-in-from-left-2 fade-in duration-300">
          <div className="bg-gray-50 border border-gray-200 rounded-xl p-3 min-w-[280px]">
            <div className="flex items-center justify-between mb-2 pb-2 border-b border-gray-200">
              ...
              <span className="text-xs font-bold bg-white px-2 rounded border border-gray-200">{colabsData.length}</span>
            </div>
            {isLoading ? (
              ...
              <div className="space-y-2 max-h-[200px] overflow-y-auto custom-scrollbar">
                {colabsData.map((colab: any) => (
                  <div
                    key={colab.id}
                    onClick={() => onViewHistory(colab.id, colab.nome)}
...
interface SectorNodeProps {
  ...
  onTriggerEditCargoDetails: (cargo: any) => void;
  onTriggerDeleteCargoLink: (linkId: number) => void;
  onTriggerAddCargoSafe: (sectorId: number) => void;
  onHandleManageExams: (sector: any) => void;
  onHandleManageRisks: (sector: any) => void;
  onTriggerPeriodicitySector: (sector: any) => void;
  onTriggerEditSector: (sector: any) => void;
        ...
        <div className="flex flex-col gap-6 relative">
          ...
          {sector.setor.cargo_setor.map((cargo: any, cIdx: number) => (
            <CargoNode
              ...
              cargo={cargo}
              unitId={unitId}
              expandedColabs={expandedColabs}
              loadingColabs={loadingColabs}
              onToggleCollaborators={onToggleCollaborators}
              onViewHistory={onViewHistory}
              onTriggerPeriodicityCargo={onTriggerPeriodicityCargo}
...
interface UnitNodeProps {
  ...
  onTriggerEditSector: (sector: any) => void;
  onTriggerDeleteSectorLink: (linkId: number) => void;
  onTriggerAddSectorSafe: (unitId: number) => void;
  onHandlePreparePCMSO: (unit: any) => void;
  onTriggerBulkUpload: (unitId: number, unitName: string) => void;
        ...
        <div className="flex flex-col gap-8 relative">
          ...
          {unit.unidade_setor.map((sector: any, sIdx: number) => (
            <SectorNode
              ...
              onTriggerEditCargoDetails={onTriggerEditCargoDetails}
              onTriggerDeleteCargoLink={onTriggerDeleteCargoLink}
              onTriggerAddCargoSafe={onTriggerAddCargoSafe}
              onHandleManageExams={onHandleManageExams}
              onHandleManageRisks={onHandleManageRisks}
              onTriggerPeriodicitySector={onTriggerPeriodicitySector}
              onTriggerEditSector={onTriggerEditSector}
...
const AsoHistoryModal = ({ isOpen, colabName, data, onClose, loading }: { isOpen: boolean, colabName: string, data: any[], onClose: () => void, loading: boolean }) => {
  ...
  return (
    <div className="fixed inset-0 z-[110] flex items-center justify-center bg-black/40 backdrop-blur-sm animate-in fade-in duration-200 p-4">
      <GlassCard className="w-full max-w-lg bg-white border-none shadow-2xl p-0 overflow-hidden">
        <div className="flex flex-col max-h-[85vh]">
...
export default function Dashboard({ session }: DashboardProps) {
  ...
  // scale is in React state so zoom controls trigger re-render of the container transform.
  // pan (x, y) is done via ref + direct DOM style mutation to avoid re-rendering the tree on every mouse move.
  ...
  // Keep a legacy canvasState shape for zoom controls that need center position.
  // const [canvasCenterState, setCanvasCenterState] = useState({ x: 0, y: 0 }); // Removed, not needed for pan anymore
  ...
  const refreshCurrentView = useCallback(async () => {
    ...
    if (view === 'hierarchy' && hierarchyCompany) {
      // Refresh data only — preserve canvas position
      await refreshHierarchyDataRef.current?.();
    }
  ...
  }, [activeTab, view, hierarchyCompany, selectedCompany, selectedUnit, selectedSector]);
  ...
  const fetchOrphanedUnits = useCallback(async () => {
  ...
  const openHierarchy = useCallback(async (company: Cliente) => {
    setView('hierarchy');
  ...
  // Keep refs in sync so refreshCurrentView can call these without forward-reference issues
  openHierarchyRef.current = openHierarchy;
  const refreshHierarchyDataRef = useRef<(() => Promise<void>) | null>(null);
  refreshHierarchyDataRef.current = refreshHierarchyData;
  ...
  const applyCanvasTransform = useCallback(() => {
    ...
    if (gridRef.current) {
      gridRef.current.style.backgroundSize = `${30 * canvasScale}px ${30 * canvasScale}px`;
  ...
  const handleMouseMove = useCallback((e: React.MouseEvent) => {
    ...
    // Mutate DOM directly — no React re-render needed for pan
    if (canvasContentRef.current) {
      canvasContentRef.current.style.transform = `translate(${panRef.current.x}px, ${panRef.current.y}px) scale(${canvasScale})`;
    }
    if (gridRef.current) {
  ...
  const handleWheel = useCallback((e: React.WheelEvent) => {
    ...
    if (e.shiftKey) {
      // Zoom with Shift + Wheel (Centered at mouse position)
      ...
      // Current point in "world" space (unscaled/untranslated)
      const wsX = (mouseX - panRef.current.x) / canvasScale;
      const wsY = (mouseY - panRef.current.y) / canvasScale;

      const scaleAdjustment = -e.deltaY * 0.001;
      const newScale = Math.min(Math.max(0.2, canvasScale + scaleAdjustment), 3);

      // Adjust pan to keep (wsX, wsY) at same (mouseX, mouseY) screen location
      panRef.current.x = mouseX - wsX * newScale;
      panRef.current.y = mouseY - wsY * newScale;

      setCanvasScale(newScale);

      // Force instant DOM update for pan part (scale will follow via useEffect/render)
      if (canvasContentRef.current) {
        canvasContentRef.current.style.transform = `translate(${panRef.current.x}px, ${panRef.current.y}px) scale(${newScale})`;
      }
      if (gridRef.current) {
        gridRef.current.style.backgroundPosition = `${panRef.current.x}px ${panRef.current.y}px`;
      }
    } else {
      // Vertical Pan with regular Wheel
  ...
  const zoomIn = useCallback(() => setCanvasScale(prev => Math.min(prev + 0.2, 3)), []);
  const zoomOut = useCallback(() => setCanvasScale(prev => Math.max(prev - 0.2, 0.2)), []);
  const centerCanvas = useCallback(() => {
    panRef.current = { x: 0, y: 0 };
    setCanvasScale(1);
    // Also reset grid/content directly
    if (canvasContentRef.current) canvasContentRef.current.style.transform = `translate(0px, 0px) scale(1)`;
    if (gridRef.current) gridRef.current.style.backgroundPosition = '0px 0px';
  }, []);
  ...
  // --- Docs Logic ---
  const triggerViewDocs = useCallback(async (unit: { id: number, nome_unidade: string }) => {
    setViewDocsModal({
      isOpen: true,
  ...
  }, [bulkUploadModal.unitId, hierarchyCompany, openHierarchy]);

  // --- User Creation Logic ---
  const triggerCreateUser = useCallback(async (company: Cliente) => {
    try {
  ...
  const handleViewHistory = useCallback(async (colabId: string, colabName: string) => {
  ...
  const handlePreparePCMSO = useCallback((unit: any) => {
...
`;
  ...
  const handlePrintFromPreview = useCallback((editedContent: string) => {
  ...
  const handleManageRisks = useCallback((sector: any) => {
    setRiskModal({
      isOpen: true,
  ...
  const handleManageExams = useCallback((sector: any) => {
    setExamModal({
      isOpen: true,
      sectorName: sector.setor.nome,
      sectorLinkId: sector.id, // This is the ID from unidad_setor table
  ...
  const toggleCollaborators = useCallback(async (unitId: number, cargoId: number) => {
              ...
              {view === 'hierarchy' ? (
                <div className="absolute inset-0 z-10 bg-[#f8fafc] flex flex-col h-full w-full">
                  ...
                  >
                    ...
                    {/* Content Container with Transform (updated via canvasContentRef directly on pan) */}
                    <div
                      ref={canvasContentRef}
                      className="absolute top-0 left-0 min-w-full min-h-full origin-top-left will-change-transform"
                      style={{
                        transform: `translate(0px, 0px) scale(${canvasScale})`,
                      }}
                    >
                      <div className="p-20 inline-block min-w-max">
                        {hierarchyLoading ? (
                          ...
                          <div className="flex items-center">
                            ...
                            {/* Units - rendered via memoized UnitNode to avoid re-render on pan/zoom */}
                            <div className="flex flex-col gap-12 relative">
                              ...
                              {hierarchyData.map((unit, uIdx) => (
                                <UnitNode
                                  ...
                                  unit={unit}
                                  uIdx={uIdx}
                                  totalUnits={hierarchyData.length}
                                  expandedColabs={expandedColabs}
                                  loadingColabs={loadingColabs}
                                  onToggleCollaborators={toggleCollaborators}
                                  onViewHistory={handleViewHistory}
                                  onTriggerPeriodicityCargo={triggerPeriodicityCargo}
                                  onTriggerEditCargoDetails={triggerEditCargoDetails}
                                  onTriggerDeleteCargoLink={triggerDeleteCargoLink}
                                  onTriggerAddCargoSafe={triggerAddCargoSafe}
                                  onHandleManageExams={handleManageExams}
                                  onHandleManageRisks={handleManageRisks}
                                  onTriggerPeriodicitySector={triggerPeriodicitySector}
                                  onTriggerEditSector={triggerEditSector}
                                  onTriggerDeleteSectorLink={triggerDeleteSectorLink}
                                  onTriggerAddSectorSafe={triggerAddSectorSafe}
                                  onHandlePreparePCMSO={handlePreparePCMSO}
                                  onTriggerBulkUpload={triggerBulkUpload}
