...
export default function AppointmentFormClient({ preSelectedColabId }: AppointmentFormClientProps) {
  ...
  const fetchClientData = async () => {
    ...
    try {
      ...
      const { data: units } = await supabase.from('unidades').select('id, nome_unidade').eq('empresaid', userProfile.cliente_id);
      const myUnits = units || [];
      setUnidades(myUnits);
